...
한 번 실행해서 docs/characters/{id}/intro.mp3 만든 후 commit + push.
...
async def amain(out_root: Path) -> int:
    ...
    for char in CHARACTERS:
        ...
        out_path = out_root / char.id / "intro.mp3"
        out_path.parent.mkdir(parents=True, exist_ok=True)
        out_path.write_bytes(result.audio_bytes)
        logging.info("  → %s (%d bytes)", out_path, len(result.audio_bytes))

    logging.info("Done. Commit and push docs/characters/*/intro.mp3 to publish.")
    return 0


def main() -> None:
    # docs/characters/{id}/intro.mp3 으로 저장. (worker/ 에서 실행 가정)
    out_root = Path(__file__).resolve().parent.parent / "docs" / "characters"
