...
void main() {
  group('outfitGuideFor', () {
    test('maps every temperature boundary to the expected guide', () {
      ...
      expect(outfitGuideFor(17).key, '17_22');
      expect(outfitGuideFor(22).key, '17_22');
    ...
    test('keeps the daily recommendation stable for the same seed', () {
      final guide = outfitGuideFor(25);
      final first = guide.recommendationFor('2026-06-05|seoul');
      final second = guide.recommendationFor('2026-06-05|seoul');

      expect(identical(first, second), isTrue);
    });

    test('contains every bundled outfit option', () {
      expect(outfitGuides.map((guide) => guide.options.length), [
        4,
        4,
        4,
        4,
